...
import { TrendingUp, TrendingDown, Package, AlertTriangle, RefreshCw, ArrowRightLeft } from 'lucide-react'
...
export default function DashboardOverview() {
  ...
  const fetchDashboardStats = async () => {
    try {
      ...
      setStats({
        shipments: { total: shipments.length, byStatus: byStatus(shipments) },
        complaints: { total: complaints.length, byStatus: byStatus(complaints, 'status') },
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
      <div className="flex items-center justify-between mb-4">
        ...
        {trend && (
          <span className={`text-sm font-medium flex items-center gap-1 ${trend.startsWith('+') ? 'text-green-600' : 'text-red-600'}`}>
            {trend.startsWith('+') ? <TrendingUp className="h-4 w-4" /> : <TrendingDown className="h-4 w-4" />}
            {trend}
          </span>
        )}
      </div>
